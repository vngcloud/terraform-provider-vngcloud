---
# generated by https://github.com/hashicorp/terraform-plugin-docs
subcategory: "vDB"
page_title: "vngcloud_vdb_postgresql_cluster_package Data Source - terraform-provider-vngcloud"
description: |-
  Provides a VNG Cloud vDB PostgreSQL Cluster Package.
---

# vngcloud_vdb_postgresql_cluster_package (Data Source)



## Example Usage

```terraform
data "vngcloud_vdb_postgresql_cluster_package" "db_s_general_1x2" {
  name = "vdb.s-general-2x4"
  zone_id = "HCM03-1A"
}
```

## Argument Reference

The following arguments are supported by the resource:

- **name** (String, Required): Name of the package
- **zone_id** (String, Optional, Computed): Zone ID of the package. Default value: HCM03-1A. Allowed values: HCM03-1A, HCM03-1B, HCM03-1C


## Attributes Reference

In addition to all arguments above, the following attributes are exported:
- **id** (String, Computed):
    - The ID of the package.

- **cpu** (Int, Computed):
    - The number of CPU cores allocated to the resource. This field is computed and available after the data source is queried.

- **ram** (Int, Computed):
    - The amount of RAM (in GB) allocated to the resource. This field is computed and available after the data source is queried.

- **backup_size** (Int, Computed):
    - The size of the backup storage (in GB) associated with the resource. This field is computed and available after the data source is queried.



